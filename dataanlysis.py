...
def data_analysis(question):
    """
    this function is to extract data from sql based on some predefined questions to analyze the data
    ...
    if question == '1. What are the names of all the videos and their corresponding channels?':
        ...
        cursor.execute(query)
        rows = cursor.fetchall()
        data = [{'Channel Name': row.channel_name, 'Video Name': row.video_name, 'View Count': row.view_count} for row
                in rows]
        df = pd.DataFrame(data)
        df.index = range(1, len(df) + 1)
        conn.close()
        ...
        cursor.execute(query)
        rows = cursor.fetchall()
        data = [{'Video Name': row.video_name, 'Comment Count': row.comment_count} for row in rows]
        df = pd.DataFrame(data)
        df.index = range(1, len(df) + 1)
        conn.close()
        ...
        cursor.execute(query)
        rows = cursor.fetchall()
        data = [{'Video Name': row.video_name, 'Channel Name': row.channel_name, 'Like Count': row.like_count} for row
                in rows]
        df = pd.DataFrame(data)
        df.index = range(1, len(df) + 1)
        conn.close()
    ...
    elif question == ('6. What is the total number of likes and dislikes for each video, and what are their '
                      'corresponding channel names?'):
        query = """SELECT v.video_name, c.channel_name, ISNULL(v.like_count,0) AS like_count , 
        ISNULL(v.dislike_count,0) AS dislike_count FROM video v JOIN playlist p ON v.playlist_id = p.playlist_id JOIN 
        channel c ON p.channel_id = c.channel_id"""
        cursor.execute(query)
        rows = cursor.fetchall()
        print(rows)
        data = [{'Video Name': row.video_name, 'Channel Name': row.channel_name, 'Like Count': row.like_count,
                 'Dislike Count': row.dislike_count} for
                row in rows]
        df = pd.DataFrame(data)
        df.index = range(1, len(df) + 1)
        conn.close()
        ...
        cursor.execute(query)
        rows = cursor.fetchall()
        data = [{'Channel Name': row.channel_name, 'Channel Views': row.channel_views} for row in rows]
        df = pd.DataFrame(data)
        df.index = range(1, len(df) + 1)
        conn.close()
        ...
        cursor.execute(query)
        rows = cursor.fetchall()
        data = [{'Channel Name': row.channel_name} for row in rows]
        df = pd.DataFrame(data)
        df.index = range(1, len(df) + 1)
        conn.close()
        ...
        cursor.execute(query)
        rows = cursor.fetchall()
        data = [{'Channel Name': row.channel_name, 'Avg duration of videos in seconds': row.average_duration} for
                row in rows]
        df = pd.DataFrame(data)
        df.index = range(1, len(df) + 1)
        conn.close()
        ...
        return df
    elif question == ('10. Which video have the highest number of comments, and what are their corresponding channel '
                      'names?'):
        query = """
        ...
        rows = cursor.fetchall()
        data = [
            {'Channel Name': row.channel_name, 'Video Name': row.video_name, 'Number of Comments': row.comment_count}
            for
            row in rows]
        df = pd.DataFrame(data)
        df.index = range(1, len(df) + 1)
...
# x = data_analysis('What is the total number of likes and dislikes for each video, and what are their corresponding
# channel names?') print(x.to_string())
